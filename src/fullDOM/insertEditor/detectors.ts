import type { DetectionResult, DetectedType, TinyMCEType, wangEditorType, CKEditor4Type, CKEditor5Type, UEType, phpBBType } from '../types';

const bodyInnerText = document.body.innerText;


// 域名检测器
const domainDetectors: Record<string, DetectedType> = {
...
const cmsDetectors: Array<() => DetectionResult | null> = [
    ...
    () => {
        const phpBB = (window as { phpbb?: phpBBType }).phpbb;
...
const editorDetectors: Array<() => DetectionResult | null> = [
    ...
    () => {
        try {
            const TinyMCE = (window as { tinymce?: TinyMCEType }).tinymce?.activeEditor;
            return TinyMCE ? {
                type: 'TinyMCE',
                certainty: 1,
    ...
    () => {
        try {
            const globalEditor = (window as { editor?: wangEditorType }).editor;
    ...
    () => {
        try {
            const ckeditor4 = (window as { CKEDITOR?: CKEditor4Type }).CKEDITOR;
