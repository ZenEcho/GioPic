<template>
    <n-form :model="formValue" ref="formRef" :rules="rules">
        ...
        <!-- 选择上传模式 图片上传或视频上传 -->
        <n-form-item label="上传模式" path="imgur_mode">
            <n-radio-group v-model:value="formValue.imgur_mode">
                <n-radio value="image">图片上传</n-radio>
                <n-radio value="video">视频上传</n-radio>
            </n-radio-group>
        </n-form-item>
        <n-form-item>
...
const formValue = ref({
    Host: 'https://api.imgur.com/3/upload',
    Token: '',
    imgur_mode: 'image',
    fullURL: false
})
...
watch(() => props.initialFormValue, (newVal) => {
    if (newVal) {
        ...
        const filteredNewVal = Object.keys(formValue.value).reduce<Record<string, any>>((acc, key) => {
            // 排除掉Host
            if (key !== 'Host' && newVal.hasOwnProperty(key)) {
                acc[key] = newVal[key];
            }
            return acc;
        }, {});
